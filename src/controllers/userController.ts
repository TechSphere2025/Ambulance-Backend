...
export const createUser = async (req: Request, res: Response) => {
  try {
    ...
    const newUser:any = await baseRepository.insert('users', { name, email }, userSchema);

    let hashedPassword = await common.hashPassword(password)
    
    await baseRepository.insert('login', { user_id: newUser.id, password: hashedPassword }, loginSchema);
